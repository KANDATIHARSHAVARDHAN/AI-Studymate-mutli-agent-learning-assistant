...
from vector_store import get_embedding_model, build_retriever, get_global_retriever
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Initialize retriever from existing Pinecone index on startup
        retriever = get_global_retriever(embedding_model)
        print("[SUCCESS] Retriever initialized from Pinecone.")
    except Exception as e:
        print(f"[ERROR] Initialization failed: {e}")
        print("   Components will be initialized on first upload.")
        embedding_model = None
        retriever = None
    yield
    # Cleanup (if needed) on shutdown
    print("[SHUTDOWN] Shutting down Multi-Agentic RAG API.")
...
async def evaluate_query(request: EvaluateRequest):
    ...
    try:
        metrics = evaluate_interaction(request.query, request.response, request.contexts)
        return metrics
    except Exception as e:
        traceback.print_exc()
